import { Class, CommandResolvers, WorkerClass } from "../interfaces";
import { Workerized } from "./../interfaces";
import { createWorker } from "./create-worker";
import { createWorkerized } from "./create-workerized";
 ...
 */
export async function workerize<W extends Object>(
  workerizedClass: Class<W>,
  workerClass: WorkerClass
): Promise<Workerized<W>> {
  const resolvers: CommandResolvers = new Map();
  const worker = await createWorker(workerizedClass, workerClass, resolvers);
  return createWorkerized(workerizedClass, worker, resolvers);
}
